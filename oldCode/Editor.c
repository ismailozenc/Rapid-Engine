...
typedef struct
{
    ...
    int scrollIndex;
    int clickedNodeID;
    int lastClickedNodeID;
    int clickedNodeIndex;
    int clickedNodeAttributeIndex;
    int hoveredItem;
    char *clickedType;
...
EditorContext InitEditorContext()
{
    ...
    EC.clickedNodeID = -1;
    EC.lastClickedNodeID = -1;
    EC.clickedNodeIndex = -1;
    EC.clickedNodeAttributeIndex = -1;
...
void FreeEditorContext(EditorContext *EC)
{
    ...
    for (size_t i = 0; i < EC->nodeCount; i++)
    {
        for (int j = 0; j < GetInputCountByType(EC->nodes[i].type); j++)
            if (EC->nodes[i].input[j])
                free(EC->nodes[i].input[j]);

        for (int j = 0; j < GetOutputCountByType(EC->nodes[i].type); j++)
            if (EC->nodes[i].output[j])
                free(EC->nodes[i].output[j]);
    }
...
void DrawNodes(Node *NodeList, size_t nodeCount)
{
    ...
    for (int i = 0; i < nodeCount; i++)
    {
        ...
        {
            char nextAttributeID[16]; // Make sure it's big enough for the number and null terminator

            sprintf(nextAttributeID, "%d", NodeList[i].next);

            linkedNodeIndex = GetLinkedNodeIndex(NodeList, nodeCount, nextAttributeID);

            if (linkedNodeIndex != -1)
            {

                char lastTwo[3];
                strncpy(lastTwo, nextAttributeID + strlen(nextAttributeID) - 2, 2);
                lastTwo[2] = '\0'; // Null-terminate

                int linkedNodeAttributeNumber = atoi(lastTwo);
        ...
        for (int j = 0; j < GetOutputCountByType(NodeList[i].type); j++)
        {
            ...
            if (strcmp(NodeList[i].output[j], "-1") != 0)
            {
                linkedNodeIndex = GetLinkedNodeIndex(NodeList, nodeCount, NodeList[i].output[j]);

                if (linkedNodeIndex != -1)
                {

                    char lastTwo[3]; // 2 digits + null terminator
                    strncpy(lastTwo, NodeList[i].output[j] + strlen(NodeList[i].output[j]) - 2, 2);
                    lastTwo[2] = '\0'; // Null-terminate

                    int linkedNodeAttributeNumber = atoi(lastTwo);

                    DrawBlueprintWire((Vector2){NodeList[i].position.x + GetWidthByType(NodeList[i].type) - 14, NodeList[i].position.y + 80 + j * 25}, (Vector2){NodeList[linkedNodeIndex].position.x + 15, NodeList[linkedNodeIndex].position.y + 80 + (linkedNodeAttributeNumber - ATTRIBUTES_SUB_ID_STARTING_POS) * 25}, 2.0f, (Color){139, 233, 253, 255});
                }
            }
        }
    }
...
int CheckOverlaps(EditorContext *EC, Node *NodeList, size_t *nodeCount)
{
    ...
    for (int i = 0; i < *nodeCount; i++)
    {
        if (CheckCollisionPointRec((*EC).mousePos, (Rectangle){NodeList[i].position.x, NodeList[i].position.y, GetWidthByType(NodeList[i].type), GetHeightByType(NodeList[i].type)}))
        {
            ...
            if (IsMouseButtonPressed(MOUSE_BUTTON_RIGHT))
            {
                ...
                for (int i = 0; i < *nodeCount; i++)
                {
                    printf("%d %d ", *nodeCount, NodeList[i].id);
                }
                printf("\n");
...
int main(int argc, char *argv[])
{
    ...
    while (!WindowShouldClose())
    {
        ...
        EC.clickedNodeID = CheckOverlaps(&EC, NodeList, &nodeCount);

        if (IsMouseButtonPressed(MOUSE_BUTTON_RIGHT) && EC.clickedNodeID == -1)
        {
            printf("%d", EC.clickedNodeID);
        ...
        else if (EC.lastClickedNodeID != -1)
        {
            for (int i = 0; i < nodeCount; i++)
            {
                if (NodeList[i].id == EC.lastClickedNodeID / 100)
                {
                    EC.clickedNodeIndex = i;
                    EC.clickedNodeAttributeIndex = EC.lastClickedNodeID % 100;
                    break;
                }
            }

            if (EC.clickedNodeAttributeIndex == 0)
            {
                DrawBlueprintWire((Vector2){EC.mousePos.x, EC.mousePos.y}, (Vector2){NodeList[EC.clickedNodeIndex].position.x + 10, NodeList[EC.clickedNodeIndex].position.y + 48}, 2.0f, (Color){209, 255, 5, 255});
            }
            else if (EC.clickedNodeAttributeIndex == 1)
            {
                DrawBlueprintWire((Vector2){NodeList[EC.clickedNodeIndex].position.x + GetWidthByType(NodeList[EC.clickedNodeIndex].type) - 5, NodeList[EC.clickedNodeIndex].position.y + 48}, (Vector2){EC.mousePos.x, EC.mousePos.y}, 2.0f, (Color){209, 255, 5, 255});
            }
            else if (EC.clickedNodeAttributeIndex < GetInputCountByType(NodeList[EC.clickedNodeIndex].type) + ATTRIBUTES_SUB_ID_STARTING_POS)
            {
                DrawBlueprintWire((Vector2){EC.mousePos.x, EC.mousePos.y}, (Vector2){NodeList[EC.clickedNodeIndex].position.x + 15, NodeList[EC.clickedNodeIndex].position.y + 80 + (EC.clickedNodeAttributeIndex - ATTRIBUTES_SUB_ID_STARTING_POS) * 25}, 2.0f, (Color){209, 255, 5, 255});
            }
            else
            {
                DrawBlueprintWire((Vector2){NodeList[EC.clickedNodeIndex].position.x + GetWidthByType(NodeList[EC.clickedNodeIndex].type) - 14, NodeList[EC.clickedNodeIndex].position.y + 80 + (EC.clickedNodeAttributeIndex - GetInputCountByType(NodeList[EC.clickedNodeIndex].type) - ATTRIBUTES_SUB_ID_STARTING_POS) * 25}, (Vector2){EC.mousePos.x, EC.mousePos.y}, 2.0f, (Color){209, 255, 5, 255});
            }
            ...
            if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
            {
                ...
                {
                    if (MakeConnection(EC.lastClickedNodeID, EC.clickedNodeID, NodeList, nodeCount) == 1)
                    {
                        printf("Cannot connect");
                    }
                    for (int i = 0; i < nodeCount; i++)
                    {
                        printf("%d %d\n", NodeList[i].prev, NodeList[i].next);
                    }
                    EC.lastClickedNodeID = -1;
                    EC.clickedNodeID = -1;
                }
        ...
        if (EC.clickedNodeID != -1)
        {
            EC.lastClickedNodeID = EC.clickedNodeID;
        }
    ...
    for (int i = 0; i < nodeCount; i++)
    {
        // Free the dynamically allocated input strings
        if (NodeList[i].input)
        {
            for (int j = 0; j < GetInputCountByType(NodeList[i].type); j++)
            {
                if (NodeList[i].input[j])
                {
                    free(NodeList[i].input[j]);
                    NodeList[i].input[j] = NULL; // Set each input to NULL after freeing
                }
            }
        }

        // Free the dynamically allocated output strings
        if (NodeList[i].output)
        {
            for (int j = 0; j < GetOutputCountByType(NodeList[i].type); j++)
            {
                if (NodeList[i].output[j])
                {
                    free(NodeList[i].output[j]);
                    NodeList[i].output[j] = NULL; // Set each output to NULL after freeing
                }
            }
        }
    }
